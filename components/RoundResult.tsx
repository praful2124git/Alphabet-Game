...
export const RoundResult: React.FC<RoundResultProps> = ({ 
  ...
  return (
    <div className="flex flex-col md:flex-row h-full max-w-6xl mx-auto p-4 md:p-8 animate-pop-in gap-4 md:gap-8 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="text-center mb-6 shrink-0">
          ...
          <div className="flex justify-center items-center gap-8 mt-2">
            <div className="bg-matte-card p-3 rounded-xl shadow-sm border border-stone-600">
               <p className="text-xs font-bold text-stone-300 uppercase">You</p>
               <p className="text-orange-400 font-black text-2xl">{validation.totalRoundScore} <span className="text-sm text-stone-400 font-normal">pts</span></p>
            </div>
            
            {isMultiplayer && (
              <div className="bg-matte-card p-3 rounded-xl shadow-sm border border-stone-600">
                <p className="text-xs font-bold text-stone-300 uppercase">Opponent</p>
                {opponentScore !== null && opponentScore !== undefined ? (
                   <p className="text-white font-black text-2xl">{opponentScore} <span className="text-sm text-stone-400 font-normal">pts</span></p>
                ) : (
      ...
      <div className="flex flex-col md:w-80 lg:w-96 shrink-0 h-64 md:h-auto border-t md:border-t-0 md:border-l border-stone-200 dark:border-stone-700 pt-4 md:pt-0 md:pl-6">
        {isMultiplayer ? (
          <div className="flex flex-col h-full bg-matte-card rounded-2xl overflow-hidden border border-stone-600 shadow-md">
             <div className="bg-stone-800/50 p-2 text-center text-xs font-bold text-stone-300 uppercase tracking-widest border-b border-stone-600">
               Live Chat
             </div>
             <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-stone-900/20">
               {chatMessages.length === 0 ? (
                 <div className="text-center text-stone-500 text-xs italic mt-4">Say hello!</div>
               ) : (
                 chatMessages.map(msg => (
                   <div key={msg.id} className={`flex flex-col ${msg.isMe ? 'items-end' : 'items-start'}`}>
                      <div className={`max-w-[85%] px-3 py-2 rounded-xl text-sm ${msg.isMe ? 'bg-orange-600 text-white rounded-tr-none' : 'bg-stone-700 text-stone-100 rounded-tl-none'}`}>
                        {msg.text}
                      </div>
             ...
             <div className="p-2 bg-stone-800/50 border-t border-stone-600 flex gap-2">
               <input
                 type="text"
                 value={msgText}
                 ...
                 className="flex-1 bg-stone-900 text-stone-100 text-sm px-3 py-2 rounded-lg outline-none focus:ring-1 focus:ring-orange-500"
               />
               <button 
                 onClick={handleSend}
    ...
    <div className={`p-4 rounded-xl border-l-8 shadow-sm flex items-center justify-between transition-colors bg-matte-card`}>
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-stone-300">{category}</span>
        </div>
        <div className="text-xl font-bold text-white break-all">
          {input || <span className="text-stone-500 italic">Empty</span>}
        </div>
        <div className={`text-sm mt-1 font-medium ${valid ? 'text-green-400' : 'text-red-400'}`}>
          {valid ? '✅ ' : '❌ '} {message}
        </div>
      </div>
      <div className="ml-4 flex flex-col items-center justify-center min-w-[3rem]">
        <span className={`text-2xl font-black ${valid ? 'text-green-400' : 'text-red-400'}`}>
          +{score}
        </span>
